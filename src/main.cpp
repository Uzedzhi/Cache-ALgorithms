...
void PrintResults(const TCacheRealHierarchy<KeyType,  PageType>& RealCacheHierarchy,
                  const TCacheIdealHierarchy<KeyType, PageType>& IdealCacheHierarchy,
...
PageType SlowGetPage(const KeyType &Key) {
    return Key;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        TCacheRealHierarchy<KeyType,  PageType>  RealCacheHierarchy(LevelConfigs);
        TCacheIdealHierarchy<KeyType, PageType> IdealCacheHierarchy(RealCacheHierarchy.GetSumCapacities(), DataStream);

        for (std::size_t i = 0; i < DataStream.size(); ++i) {
            int Key = DataStream[i];

            RealCacheHierarchy.FindInsert(Key, SlowGetPage);
            IdealCacheHierarchy.FindInsert(Key, i, SlowGetPage);
        }
